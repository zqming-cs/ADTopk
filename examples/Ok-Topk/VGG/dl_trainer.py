...
_support_datasets = ['imagenet', 'cifar10', 'cifar100', 'an4', 'ptb', 'mnist']
...
def create_net(num_classes, dnn='resnet20', **kwargs):
    ...
    if dnn in ['resnet20', 'resnet56', 'resnet110']:
        ...
        # cifar10
        # net = models.VGG(dnn.upper(),10)
        
        # cifar100
        # net = models.VGG(dnn.upper(),100)
        
        from model_utils import get_network
        net = get_network('vgg16')
        
        # import torchvision.models as models
        # net = models.vgg16_bn(num_classes=1000)
...
class DLTrainer:
    ...
    def cifar10_prepare(self):
        ...
        train_sampler = None
        shuffle = True
        # print('self.nworkers', self.nworkers)
    ...
    def cifar100_prepare_back(self):
        # image_size = 32
        # self._input_shape = (self.batch_size, 3, image_size, image_size)
        # self._output_shape = (self.batch_size, 100)
        ...
        trainset = \
        torchvision.datasets.CIFAR100('/home/mzq/cifar100/train',
                             train=True,
                             download=True,
        ...
        testset = torchvision.datasets.CIFAR100('/home/mzq/cifar100/validation',
                             train=False,
                             download=True,
                             transform=transforms.Compose([
        ...
        self.trainloader = torch.utils.data.DataLoader(trainset, batch_size=self.batch_size,
                                                #   num_workers=0, sampler=train_sampler)
                                                  shuffle=shuffle, num_workers=NUM_CPU_THREADS, pin_memory=True, sampler=train_sampler)
        self.testloader = torch.utils.data.DataLoader(testset, batch_size=self.batch_size,
                                                #  num_workers=0)
    ...
    def an4_prepare(self):
        ...
        self.train_sampler = train_sampler
        trainloader = AudioDataLoader(self.trainset, num_workers=0, batch_sampler=self.train_sampler)
    ...
    def _adjust_learning_rate_general(self, progress, optimizer):
        ...
        if self.dataset == 'imagenet' or 'cifar10' or 'cifar100':
            first = 30
            # first = 40
            second = 60
            third = 80
    ...
    # def adj_lr(epoch, batch_idx):
    #     if epoch < args.warmup_epochs:
    #         epoch += float(batch_idx + 1) / len(train_loader)
    #         lr_adj = 1. / self.size() * (epoch * (hvd.size() - 1) / args.warmup_epochs + 1)
    ...
    def train_oktopk(self, num_of_iters=1, curr_iter=0, data=None, hidden=None):
        self.loss = 0.0
        s = time.time()

        for i in range(num_of_iters):
            self.adjust_learning_rate(self.train_epoch, self.optimizer)
            if curr_iter==self.num_batches_per_epoch and self.train_iter > 0 and self.rank == 0:
            # if self.train_iter % self.num_batches_per_epoch == 0 and self.train_iter > 0 and self.rank == 0:
                ...
                avg_train_acc = np.mean(self.train_acc_top1)
                avg_train_loss = self.avg_loss_per_epoch/self.num_batches_per_epoch
                
                self.avg_train_acc_array.append(avg_train_acc)
                self.avg_train_loss_array.append(avg_train_loss)
        
                logger.info('Epoch %d, avg train acc: %f, lr: %f, avg loss: %f' % (self.train_epoch, avg_train_acc, self.lr, avg_train_loss))
                mean_s = np.mean(self.sparsities)
                
                # if self.train_iter>0 and np.isnan(mean_s) and self.rank == 0 :
                ...
                if self.rank == 0 :
                    ...
                    test_acc, test_loss=self.test(self.train_epoch+1)
                    self.test_acc_array.append(test_acc)
            ...
            if self.is_cuda:
                if self.dnn == 'lstm' :
                    ...
                    inputs, labels = inputs.cuda(), labels_cpu.cuda()
        ...
        if self.train_iter % display == 0 and self.rank == 0:
            ...
            logger.info('GPU memory usage memory_allocated: %d MBytes, max_memory_allocated: %d MBytes, memory_cached: %d MBytes, max_memory_cached: %d MBytes, CPU memory usage: %d MBytes', 
                    ct.memory_allocated()/mbytes, ct.max_memory_allocated()/mbytes, ct.memory_reserved()/mbytes, ct.max_memory_reserved()/mbytes, process.memory_info().rss/mbytes)
        ...
        for batch_idx, data in enumerate(self.testloader):
            ...
            if self.is_cuda:
                if self.dnn == 'lstm' :
                    ...
                    # inputs, labels_cpu = Variable(inputs), Variable(labels_cpu)-1
                    inputs, labels = inputs.cuda(non_blocking=True), labels_cpu.cuda(non_blocking=True)
                    
                    inputs, labels_cpu = Variable(inputs), Variable(labels)
            else:
                labels = labels_cpu
