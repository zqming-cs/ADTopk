...
class AllReducer():
    ...
    def _print_profiling(self):
        ...
        if self._profiling and len(self._allreduce_timers.keys()) > 0 and len(self._allreduce_timers.get(list(self._allreduce_timers.keys())[0], [])) == 50 and self.rank==0:
            ...
            l = 0
            logger.info('[rank:%d]name[size:%d]: backward, merge, compression, allreduce, demerge, d2h, h2d',self.rank(),self.size())
            total_sz = 0
            total_ct = 0.0
            total_mg = 0.0
            total_cp = 0.0
    ...
    def run(self):
        ...
        while self._running:
            ...
            if name == 'STOP':
                break
            if name is not None:
                # print('self._sparse :', self._sparse )
                # print('self._compression.name :', self._compression.name )
                ...
                stime = time.time()
                # 迭代前期使用使用dense_allreduce
                # if self._allreduce_counter[new_name] < 512: 
                #     print('_allreduce_counter:', '_allreduce_counter')
                #     result = self._dense_allreduce(new_name, new_tensor)
 
 
                if self._sparse and self._compression.name == 'oktopk':
