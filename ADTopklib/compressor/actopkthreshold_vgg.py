...
class ACTopkThresholdVGGCompressor(Compressor):
    ...
    # 自适应更新阈值Vgg-16
    def Adaptive_Threshold_Estimation(self, name, tensor_flatten, tensor, shape, numel, compress_ratio, epoch, iteration):
        # Iteration的前期不进行阈值更新,直接用Topk计算Value和Indicate
        # if epoch <self.training_epochs/4:
        
        if self.allchannel[name]:
        ...
        # if epoch < 2:
            ...
            if epoch > 3/4*self.training_epochs:
                self.allchannel[name]=True
            
            # Thresholds are regularly updated
            if epoch % self.tau==0:
                k= max(1, int(numel * compress_ratio))
                values_flatten, indices_flatten_global = torch.topk(tensor_flatten.abs(), k, sorted=False,)
                values_flatten_global = torch.gather(tensor_flatten, 0, indices_flatten_global)
                ...
                # update step tau
                if torch.abs(thres_global-thres) < thres/10:
                    self.tau=self.tau + 1
                elif torch.abs(thres_global-thres) < thres/20:
                    self.tau=self.tau + 5
                elif torch.abs(thres_global-thres) > thres:
                    self.tau=max(self.tau - 1,5)
                elif torch.abs(thres_global-thres) > 2*thres:
    ...
    def sparsify(self,tensor, compress_ratio,epoch, iteration, name):
        ...
        if tensor.dim() >1:
            ...
            if self.attributes[name]['sign']==-1 or 'classifier.6' in name:
                ...
                values_flatten_global, indices_flatten_global= self.Adaptive_Threshold_Estimation(name, tensor_flatten, tensor, shape, numel, compress_ratio, epoch, iteration)
